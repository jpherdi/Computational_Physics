...
double simpson(double a, double b, int n)
{
    ...
    h=(b-a)/n;
    s=f(a)+f(b)+4*f(a+h/2);
    for(i=1; i<=n-1; i++) s=s+2*f(a+i*h)+4*f(a+i*h+h/2);
...
double simpson_ii(double a, double b, int n)
{
    ...
    h=(b-a)/n;
    s=g(a)+g(b)+4*g(a+h/2);
    for(i=1; i<=n-1; i++) s=s+2*g(a+i*h) + 4*g(a+i*h+h/2);
